...
;; for speed check, using queue instead of heap, results in bfs rather than dijkstra
(define (p-dij cost-of edges-from s)
  (let* [[ Q (rlet1 Q (make-queue)
               ($ enqueue! Q $ cons s 0)) ]
         ...
         (for-each
           (^ (t)
              (or (hash-table-exists? V t)
                  ($ enqueue! Q $ cons t $ + Cs $ cost-of s t)))
...
(define (solve N)
  (list-ec
    (: s N)
    (:let fw-costs (p-dij fw->cost fw-edges s))
    (:let rv-costs (p-dij rv->cost rv-edges s))
    (min-ec
      (: t N)
      (cond
        [(= s t)
